...
function parseHeistBlueprint (section: string[], item: ParsedItem) {
  ...
  for (const line of section) {
    if (line.startsWith(_$.HEIST_TARGET)) {
      ...
      const wingParts = line.slice(_$.HEIST_WINGS_REVEALED.length).split('/')
      item.heist.wingsRevealed = parseInt(wingParts[0], 10)
      item.heist.wingsTotal = parseInt(wingParts[1], 10)
    }
  }
...
function parseMirroredTablet (section: string[], item: ParsedItem) {
  ...
  for (const line of section) {
    const found = tryParseTranslation({ string: line, unscalable: true }, {
      info: { tags: [], type: ModifierType.Pseudo },
      stats: []
    })
    if (found) {
